...
import { 
  ...
  Trash2, ChevronsUpDown, Check, Building2
} from 'lucide-react';
...
function Dashboard() {
  ...
  const [drafts, setDrafts] = useState<SchoolData[]>(() => {
    try {
      const saved = localStorage.getItem('akta-sd-drafts');
  ...
  React.useEffect(() => {
    localStorage.setItem('akta-sd-drafts', JSON.stringify(drafts));
  }, [drafts]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white transition-colors duration-300 font-sans selection:bg-primary-200 selection:text-primary-900 dark:selection:bg-primary-900/80 dark:selection:text-primary-100 overflow-hidden relative flex flex-col">
      ...
      <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-indigo-500/20 dark:bg-indigo-600/20 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="absolute top-[20%] right-[10%] w-[400px] h-[400px] bg-emerald-500/10 dark:bg-emerald-600/10 rounded-full blur-[100px] pointer-events-none"></div>
        ...
        <header className="bg-white/60 dark:bg-white/5 backdrop-blur-xl shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] dark:shadow-none rounded-lg md:rounded-xl border border-white/40 dark:border-white/10 p-4 md:p-5 mx-auto w-full mb-4 shrink-0 transition-colors">
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Database Registrasi AKTA-SD</h1>
                <div className="flex items-center gap-3 mt-1.5">
                  {user ? (
                    <span className="flex items-center gap-2 px-2.5 py-1 bg-emerald-500/10 text-emerald-400 text-[10px] font-bold rounded-md border border-emerald-500/20 uppercase tracking-widest">
